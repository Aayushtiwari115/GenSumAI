...
class ToolTip:
    ...
    def _show(self):
        ...
        lbl = ctk.CTkLabel(self._tip, text=self.text, font=THEME["FONT_SM"], corner_radius=6, fg_color=("gray85", THEME["CARD_DARK"]))
...
def setup_layout(app):
    ...
    # Sidebar
    app.sidebar = ctk.CTkFrame(app, width=THEME["SIDEBAR_WIDTH"], corner_radius=8, fg_color=("gray90", THEME["CARD_DARK"]))
    ...
    if app.icons.get("logo"):
        ctk.CTkLabel(top_logo, image=app.icons["logo"], text="").pack(side="left", padx=(4,10))
    app.title_label = ctk.CTkLabel(top_logo, text=THEME["APP_NAME"], font=THEME["FONT_LG"], text_color=(THEME["TEXT_LIGHT"], THEME["TEXT_DARK"]))
    ...
    ctk.CTkLabel(task_container, text="TASKS", font=THEME["FONT_SM"], text_color=("gray60","gray70")).pack(anchor="w", pady=(0,8))
    app.task_buttons = {}
    for task in app.models.keys():
        icon = app.icons.get("gen") if "Text" in task else app.icons.get("sum")
        btn = ctk.CTkButton(task_container, text=task, image=icon, compound="left",
                            width=THEME["SIDEBAR_WIDTH"]-32, height=42, anchor="w",
    ...
    app.dark_mode_var = ctk.BooleanVar(value=(ctk.get_appearance_mode().lower()=="dark"))
    switch = ctk.CTkSwitch(sidebar_bottom, text="Dark Mode", variable=app.dark_mode_var, command=app.toggle_mode)
    switch.pack(side="left")
    if app.icons.get("settings"):
        btn = ctk.CTkButton(sidebar_bottom, width=36, height=36, image=app.icons["settings"], fg_color="transparent", hover_color=("gray85","gray25"), command=app.open_settings)
    ...
    ctk.CTkLabel(left_nav, text="Natural Language Processing", font=THEME["FONT_LG"], text_color=(THEME["TEXT_LIGHT"], THEME["TEXT_DARK"])).pack(anchor="w")

    center_nav = ctk.CTkFrame(nav, fg_color="transparent")
    center_nav.pack(side="left", expand=True)
    # model selector uses model names from app.models
    model_names = [m.get_model_name() if hasattr(m, "get_model_name") else str(m) for m in app.models.values()]
    app.model_selector = ctk.CTkOptionMenu(center_nav, values=model_names, width=260, command=app.on_model_selected)
    # set default to model for current default task
    try:
    ...
    app.task_segment = ctk.CTkSegmentedButton(center_nav, values=list(app.models.keys()), command=app.select_task)
    ...
    app.run_button = ctk.CTkButton(right_nav, text="Run", width=100, command=app.run_model, image=app.icons.get("run"))
    app.run_button.pack(side="left", padx=(6,12))
    ToolTip(app.run_button, "Run the selected task (Ctrl+R)")
    clear_btn = ctk.CTkButton(right_nav, text="Clear", width=80, command=app.clear_all, image=app.icons.get("clear"))
    clear_btn.pack(side="left", padx=6)
    ToolTip(clear_btn, "Clear input & output (Ctrl+L)")

    # Menu button (opens separate window)
    menu_btn = ctk.CTkButton(right_nav, text="Menu", width=80, command=app.open_menu_window)
    ...
    app.input_frame = ctk.CTkFrame(left_col, fg_color=("gray95", THEME["CARD_DARK"]), corner_radius=8)
    ...
    app.input_label = ctk.CTkLabel(input_header, text="Input Text", font=THEME["FONT_MD"], text_color=(THEME["TEXT_LIGHT"], THEME["TEXT_DARK"]))
    ...
    ctk.CTkLabel(toolbar, text="Max length:", font=THEME["FONT_SM"]).pack(side="left", padx=(0,6))
    ctk.CTkEntry(toolbar, textvariable=app.max_len, width=80, height=32).pack(side="left", padx=(0,12))
    ctk.CTkLabel(toolbar, text="Min length:", font=THEME["FONT_SM"]).pack(side="left", padx=(0,6))
    ctk.CTkEntry(toolbar, textvariable=app.min_len, width=80, height=32).pack(side="left", padx=(0,12))
    ...
    app.status_left = ctk.CTkLabel(status_frame, text="Ready", font=THEME["FONT_SM"], text_color=("gray60","gray70"))
    app.status_left.pack(side="left")

    # Output card
    app.output_frame = ctk.CTkFrame(left_col, fg_color=("gray95", THEME["CARD_DARK"]), corner_radius=8)
    ...
    app.output_label = ctk.CTkLabel(output_header, text="Output", font=THEME["FONT_MD"], text_color=(THEME["TEXT_LIGHT"], THEME["TEXT_DARK"]))
    app.output_label.pack(side="left")
    actions = ctk.CTkFrame(output_header, fg_color="transparent")
    actions.pack(side="right")
    ctk.CTkButton(actions, text="", width=36, height=36, image=app.icons.get("copy"), fg_color="transparent", command=app.copy_output).pack(side="left", padx=6)
    ctk.CTkButton(actions, text="", width=36, height=36, image=app.icons.get("close"), fg_color="transparent", command=lambda: app.output_box.delete("1.0", "end")).pack(side="left")
    ...
    app.right_panel = ctk.CTkFrame(content, width=320, fg_color=("gray95", THEME["CARD_DARK"]), corner_radius=8)
    ...
    app.activity_list = ctk.CTkScrollableFrame(app.right_panel, fg_color="transparent", height=420)
    app.activity_list.pack(fill="both", expand=True, padx=P, pady=(0,P))

    def add_activity(text):
        item = ctk.CTkLabel(app.activity_list, text=text, anchor="w", wraplength=260, font=THEME["FONT_SM"])
        item.pack(fill="x", pady=6, padx=6)
    add_activity("Welcome to NeuralFlow Studio — recent actions will appear here.")
    add_activity("Tip: Use Ctrl+R to run, Ctrl+L to clear, Ctrl+C to copy output, Ctrl+T to toggle input.")

    # Status bar
    statusbar = ctk.CTkFrame(main, height=36, fg_color=("gray95", THEME["CARD_DARK"]))
    statusbar.pack(fill="x", pady=(12,0))
    statusbar.pack_propagate(False)
    app.status_left = ctk.CTkLabel(statusbar, text="Ready", font=THEME["FONT_SM"], anchor="w")
    app.status_left.pack(side="left", padx=P//2)
    app.status_right = ctk.CTkLabel(statusbar, text="Idle", font=THEME["FONT_SM"], anchor="e")
